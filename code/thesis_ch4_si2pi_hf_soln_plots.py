...
#    py.title('$\left\\|\\varphi\\right\\|$ in linear $\sigma$-model ($N=%d$, $\lambda=%.f$, $v=%.f\ \mathrm{MeV}$)' % (N, lam, np.sqrt(vb2)))
    py.xlabel('T (MeV)')
    py.ylabel('$v$ (MeV)')
    py.grid(False)
    ...
    py.legend(('2PI',
               'SI-2PI',
               'symmetric phase (all)'), loc = 'best')
#    py.title('``Higgs\'\' mass in linear $\sigma$-model ($N=%d$, $\lambda=%.f$, $v=%.f\ \mathrm{MeV}$)' % (N, lam, np.sqrt(vb2)))
    py.xlabel('T (MeV)')
    py.ylabel('$m_H$ (MeV)')
    py.grid(False)
    py.annotate('Critical Temperature\n$T_c = \\sqrt{\\frac{12 v^2}{N + 2}}$', xy=(crit_T, 0),  xycoords='data',
    ...
    py.legend(('2PI',
               'SI-2PI',
               'symmetric phase (all)'), loc = 'best')
#    py.title('Goldstone mass in linear $\sigma$-model ($N=%d$, $\lambda=%.f$, $v=%.f\ \mathrm{MeV}$)' % (N, lam, np.sqrt(vb2)))
    py.xlabel('T (MeV)')
    py.ylabel('$m_G$ (MeV)')
    py.grid(False)
